...
class CheckInsert extends TestCase
{
    ...
    public function testTableRead()
    {
        ...
        $hit = 0;
        $lisa = mysqli_query($conn, "SELECT COUNT(*) FROM siswa WHERE nama_depan LIKE '%lisa%' ");
        $hit = mysqli_num_rows($lisa) == 1 ? $hit+1 : $hit+0;

        $joshua = mysqli_query($conn, "SELECT COUNT(*) FROM siswa WHERE nama_depan LIKE '%joshua%' ");
        $hit = mysqli_num_rows($joshua) == 1 ? $hit+1 : $hit+0;

        $dion = mysqli_query($conn, "SELECT COUNT(*) FROM siswa WHERE nama_depan LIKE '%dion%' ");
